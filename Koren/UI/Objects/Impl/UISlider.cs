...
public class UISlider : UIObject {
    ...
    // isn't actively typing — otherwise a slider drag (or any external
    // setter) would clobber the in-progress edit.
    ...
    // user's edit, so "8 px", "8px", "  8.5 x  " all work and trailing units
    // are tolerated.
    public static bool TryParseValue(string text, out float value) {
        ...
        foreach(char c in text) {
            if(sb.Length == 0 && (c == '-' || c == '+')) {
                ...
                // We've started reading the number — anything non-numeric
                // means the number is done (e.g., the " px" suffix).
                break;
            }
        }
